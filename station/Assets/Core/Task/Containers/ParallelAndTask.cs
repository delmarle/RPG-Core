...
namespace Station
{
    public class ParallelAndTask : BasicTaskContainer
    {
        ...
        private IEnumerator CheckChildTasks()
        {
            while (Status == TaskStatus.Running || Status == TaskStatus.Resumed)
            {
                ...
                while (_tasksFinished != _tasksCount)
                {
                    yield return null;
                }
        ...
        private void ExecuteEachChildtask()
        {
            if (_tasksEnumerator.Current != null)
            {
                ...
                _tasksEnumerator.Current.SetEndCallbackInternal(OnChildTaskFinished(_tasksEnumerator.Current))
                    .SetProgressCallback(OnChildTaskProgress(_tasksEnumerator.Current));
        ...
        private StationAction<ITask, float> OnChildTaskProgress(ITask childTasl)
        {
            return (task, progress) =>
            {
                _tasksProgressCallbacks[childTasl] = progress;
                SendBackProgress();
            };
        }

        private void SendBackProgress()
        {
            float currentProgress = _tasksProgressCallbacks.Values.Sum() / _tasksCount;
